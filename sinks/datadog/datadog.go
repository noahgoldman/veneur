...
func (dd *DatadogMetricSink) FlushEventsChecks(ctx context.Context, events []samplers.UDPEvent, checks []samplers.UDPServiceCheck) {
	...
	if len(events) != 0 {
		...
		// it still works
		err := vhttp.PostHelper(context.TODO(), dd.HTTPClient, dd.traceClient, fmt.Sprintf("%s/intake?api_key=%s", dd.DDHostname, dd.APIKey), map[string]map[string][]samplers.UDPEvent{
			"events": {
				"api": events,
			},
	...
	if len(checks) != 0 {
		...
		err := vhttp.PostHelper(context.TODO(), dd.HTTPClient, dd.traceClient, fmt.Sprintf("%s/api/v1/check_run?api_key=%s", dd.DDHostname, dd.APIKey), checks, "flush_checks", false, dd.log)
		if err == nil {
...
func (dd *DatadogMetricSink) flushPart(ctx context.Context, metricSlice []DDMetric, wg *sync.WaitGroup) {
	defer wg.Done()
	vhttp.PostHelper(ctx, dd.HTTPClient, dd.traceClient, fmt.Sprintf("%s/api/v1/series?api_key=%s", dd.DDHostname, dd.APIKey), map[string][]DDMetric{
		"series": metricSlice,
	}, "flush", true, dd.log)
}
...
func (dd *DatadogSpanSink) Flush() {
	...
	if len(finalTraces) != 0 {
		...
		err := vhttp.PostHelper(context.TODO(), dd.HTTPClient, dd.traceClient, fmt.Sprintf("%s/spans", dd.traceAddress), finalTraces, "flush_traces", false, dd.log)
